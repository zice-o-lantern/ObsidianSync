compile = (input, context) => {
  ...
  if (context.kind === "Scene") {
    return input.map((sceneInput) => {
      return {
        ...sceneInput,
        contents: sceneInput.contents.replace(HORIZONTAL_RULE_REGEX, () => "")
      };
    });
  } else {
    return {
      ...input,
      contents: input.contents.replace(HORIZONTAL_RULE_REGEX, () => "")
    };
  }
}
...
module.exports = {
  ...
  description: {
    ...
    name: "Remove Horizontal Rule",

    // short description of what it does
    description: "Remove horizontal rules with --- ",
